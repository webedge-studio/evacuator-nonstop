    ...
    >
      <div className="container grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-0">
        <div className="flex flex-col gap-4 md:mt-6">
          <div>
            <h1 className={"font-montserrat text-4xl font-bold md:text-4xl md:leading-[58px] lg:text-5xl"}>
              Evacuator Auto
            </h1>
            <h2 className={"font-montserrat text-4xl leading-[44px] font-bold xl:text-5xl xl:leading-[58px]"}>
              Nonstop în Chișinău
            </h2>
          </div>

          <p className={"max-w-[467px] text-sm leading-[24px] font-medium text-black/70 lg:text-base"}>
            Servicii rapide de evacuare și tractare auto pentru orice tip de vehicul. Ajungem în 15–20 minute în
            Chișinău și suburbii, 24/7.
          </p>

          <div className={"mt-2 flex flex-col items-start gap-4 py-4 md:py-7"}>
            <AppButton
              icon={<Image src={phone} alt="" aria-hidden className={"h-3.5 w-3.5"} />}
            ...
            />
            <AppButton
              icon={<Image src={whatsapp} alt="" aria-hidden className={"h-3.5 w-3.5"} />}
              label={"WhatsApp"}
              linkRef="tel:+37369640892"
